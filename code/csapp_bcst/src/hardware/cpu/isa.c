...
// handler table storing the handlers to different instructions types
...
static inline void next_rip(core_t *cr)
{
    ...
    // but their size can be variables as true x86 instructions
...
static void sub_handler(od_t *src_od, od_t *dst_od, core_t *cr)
{
    ...
    if (src_od->type == IMM && dst_od->type == REG)
    {
        ...
        cr->flags.ZF = (val == 0);
        cr->flags.SF = val_sign;
        cr->flags.OF = (src_sign == 1 && dst_sign == 0 && val_sign == 1) || (src_sign == 0 && dst_sign == 1 && val_sign == 0);  // signed
...
static void jne_handler(od_t *src_od, od_t *dst_od, core_t *cr)
{
    ...
    // src_od is actually a instruction memory address number
...
// instruction cycle os implemented in CPU
