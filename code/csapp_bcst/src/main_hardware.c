...
// cannot run because the readinst_dram() function is changed
static void TestAddFunctionCallAndComputation()
{
    ACTIVE_CORE = 0x0;
    ...
    */
    ...
    for (int i = 0; i < 15; ++ i)
    {
        writeinst_dram(va2pa(i * 0x40 + 0x00400000, cr), assembly[i], cr);  // 0x40 => 64
    }
    ...
    match = match && (read64bits_dram(va2pa(0x7ffffffee110, cr), cr) == 0x000000000);  // rbp, top of stack
    match = match && (read64bits_dram(va2pa(0x7ffffffee108, cr), cr) == 0x1234abcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee100, cr), cr) == 0x12340000);
    match = match && (read64bits_dram(va2pa(0x7ffffffee0f8, cr), cr) == 0x0000abcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee0f0, cr), cr) == 0x00000000);  // rsp, bottom of stack    
