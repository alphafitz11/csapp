...
static void TestAddFunctionCallAndComputation()
{
    ...
    */
    ...
    match = match && (read64bits_dram(va2pa(0x7ffffffee110, ac), ac) == 0x000000000);  // rbp, top of stack
    match = match && (read64bits_dram(va2pa(0x7ffffffee108, ac), ac) == 0x1234abcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee100, ac), ac) == 0x12340000);
    match = match && (read64bits_dram(va2pa(0x7ffffffee0f8, ac), ac) == 0x0000abcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee0f0, ac), ac) == 0x00000000);  // rsp, bottom of stack    
